...
func (i *Download) Download(ctx context.Context, sdk, version string) (archive afero.File, err error) {
	dl, err := i.Options.Client.Download.DownloadSDK(ctx, sdk, version, system.GetCurrent())
	if err != nil {
		return nil, errors.Wrap(err, "error downloading sdk from api.sdkman.io")
	}
	archivePath := i.Options.Pather.Download(sdk, version, fmt.Sprintf("%s-%s.zip", sdk, version))
	archive, err = saveArchive(i.Options.Fs, dl.Buffer, archivePath)
	if err != nil {
